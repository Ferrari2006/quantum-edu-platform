...
- **引导式线路任务**：提供5个带目标、脚手架、提示、自动验收和反思问题的实验，并保存完成数、尝试数和最佳保真度。
...
- **学习画像与推荐**：统一记录资料阅读和线路实验等学习证据，汇总概念掌握度，并在知识库首页推荐下一步主题。
- **小测与游戏证据**：5篇正式示例文章提供概念小测；两个游戏按匹配度或保真度把结算结果回传学习画像。
- **学习档案**：集中展示概念掌握度、证据时间线和下一步行动，并支持导出个人学习数据。
- **前置概念推荐**：根据知识依赖、薄弱点和课程顺序，推荐文章、线路实验、游戏或AI讲解。
...
quantum-edu-platform/
  backend/
    ...
    api/
      ...
      learning_routes.py    # 学习事件、掌握度与推荐接口
      game_routes.py        # Web 游戏桥接接口
    db.py                   # SQLite 用户、问答、学习事件与掌握度
    learning/
      catalog.py            # 知识库概念目录
      service.py            # 学习证据汇总与推荐逻辑
    rag/
  ...
  frontend/
    src/
      ...
      data/
        ...
        knowledgeQuizzes.js # 正式示例文章的概念小测题库
        quantumLabTasks.js  # 引导式线路任务与自动验收规则
      utils/
        learningEvidence.js # 小测计分与游戏结算事件映射
      components/
        NavBar.jsx
    package.json
...
- `api/learning_routes.py`：登录用户的学习事件、学习时间线、概念掌握度和推荐接口。
- `db.py`：本地 SQLite 数据层，保存用户、会话、学习记忆、问答历史、学习事件与概念掌握度；数据库文件不会进入版本库。
- `learning/`：知识库概念目录、学习证据权重、画像汇总和课程推荐逻辑。
...
- `pages/KnowledgeBase.jsx`：面向学习者的知识库前台，包含六阶段学习目录、站内搜索、文章阅读、收藏、完成进度、个人学习画像和下一步推荐。
- `data/knowledgeContent.js`：知识库的前端内容模型。当前提供 21 个主题框架和 5 篇结构示例，后续可替换为后端 Markdown 内容接口。
- `data/knowledgeQuizzes.js`：为5篇正式示例文章配置的15道概念检查题。
- `pages/OAPage.jsx`：多智能体问答页面，支持任务模式选择、Qiskit 代码输入、当前游戏状态自动附带和处理流程展示。
- `pages/QuantumLab.jsx`：图形化线路实验室，提供量子门工具箱、线路画布、引导任务、自动验收、结果图表和AI解释入口。
- `data/quantumLabTasks.js`：5个双语教学任务及其目标态、脚手架、关键门和保真度验收规则。
- `pages/LearningProgressPage.jsx`：个人学习档案，展示掌握度、行动推荐、证据时间线，并支持导出JSON。
...
- 桌面端固定目录与移动端抽屉目录。
- 关键词搜索、文章收藏、完成状态和学习进度；本地状态支持未登录使用，登录后文章完成事件会同步至学习画像。
- 登录用户可查看概念平均掌握度、课程覆盖度和下一步推荐；线路实验结果也会作为学习证据参与汇总。
- 5篇正式示例文章各包含3道概念小测，逐题解释并按真实正确率更新掌握度。
- 两个游戏拥有独立运行编号，并在关卡结算时将目标概率匹配度或态保真度回传学习画像。
- 独立学习档案页汇总全部学习证据，可筛选薄弱/熟练概念并导出个人数据。
- 第一版前置概念图会优先推荐已经具备基础的新主题，并为未满足条件的主题解释缺失概念。
- 线路类推荐会直接打开对应的引导任务；成功与失败尝试均形成可追踪的学习证据。
- 登录用户回到实验室时可看到5项任务的完成数、每项尝试次数和历史最佳保真度。
...
- [x] 将学习内容、问答、练习和游戏表现汇总为概念掌握度与教学推荐闭环MVP。
- [x] 实现通用量子运行/保真度接口和图形化量子线路实验室MVP。
- [ ] 继续扩展线路实验室：引导任务MVP已完成，后续增加三维Bloch球、测量/经典寄存器与噪声模型。
