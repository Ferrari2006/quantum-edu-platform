...
调用方把当前回合、可用卡牌/量子门、目标态、分数、限制条件等放入 `game_state`。模型只依据该状态和检索资料给出下一步动作、理由、风险及关联量子概念。AI 伴学页面选择“游戏关卡攻略”后，会在提交问题时读取 `/api/quantum-game/state`，压缩为必要字段并自动附带；其他 API 调用方也可以自行传入该字段。
...
- 游戏攻略依赖准确的实时状态；前端会自动附带当前进程内状态，但后端重启后需要重新开始游戏。
